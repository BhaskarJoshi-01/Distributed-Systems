...
int main(int argc, char **argv)
{
    ...
    MPI_Scatterv(A_matrix, chunks_per_proc, displacement_vector, MPI_LONG_DOUBLE, proc_rows, A_SZ, MPI_LONG_DOUBLE, root_rank, comm);
    // cerr << "hi " << rank << endl;
    ...
    while (cnt <= rows_per_proc - 1)
    {
        ...
        {
            MPI_Send(send_buf, REC_SZ, MPI_LONG_DOUBLE, next_proc, curr, comm);
            // cerr<<"dd\n"<<rank<<endl;
        }
    ...
    while (cnt >= 0)
    {
        ...
        {
            MPI_Send(&ans, count, MPI_LONG_DOUBLE, prev_proc, no_of_eq + lst, comm);
        }
        prev_lst = lst;
        cnt--;
        lst = lst - no_of_processes;
    }
    MPI_Gatherv(proc_vals, rows_per_proc, MPI_LONG_DOUBLE, res, chunks_per_proc, displacement_vector, MPI_LONG_DOUBLE, root_rank, comm);
